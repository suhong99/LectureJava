...
public class AccountDao {
	...
	public void insert() {
		...
		public List<Integer> search(String type ) {
			if(type.equals("확인")) {
				System.out.print(type+"할 방법(용도 : 1, 기간 : 2, 월별결산 :3)");
			...
			if(searchType==1) {
				...
				System.out.print("검색하시 연월을 입력해주세요(ex:0102 :");
				while(true) {
					searchThings = sc.next();
					// 길이가 6이 아니면 다시 반복문의 처음으로 돌아감
					 if (searchThings.length() != 4) {
			                System.out.println("4글자 짜리 연월를 입력해주세요 : ");
			                continue; 
			            }
		    ...
		    if (searchType == 1) { // 용도로 검색
		        for (int i = 0; i < s.accountList.size(); i++) {
		    	 ...
		    	 for (int i = 0; i < s.accountList.size(); i++) {
			            AccountDto dto = s.accountList.get(i);
			            int startDate = Integer.parseInt(searchThings+"01");
			            int endDate = Integer.parseInt(searchThings+"31");
